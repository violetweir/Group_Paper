...
We evaluate the segmentation framework on four public binary medical image segmentation datasets covering three clinical scenarios. BUSI~\cite{al2020dataset} contains breast ultrasound images with benign and malignant tumor cases. ISIC2018~\cite{codella2019skin} contains dermoscopic skin lesion images with pixel-level lesion masks. CVC-ClinicDB~\cite{bernal2015clinicdb} and CVC-ColonDB~\cite{tajbakhsh2016colondb,vazquez2017benchmark} contain colonoscopic polyp images collected from different video sources. These datasets jointly test low-contrast boundaries, irregular lesion shapes, texture ambiguity, and cross-source appearance shifts.

Following the MK-UNet evaluation protocol~\cite{rahman2025mkunet}, Dice score is used as the primary metric, and model efficiency is reported with parameters, FLOPs, and throughput. The current draft uses the comparison results reported in the original MK-UNet paper as reference baselines. The validation-selected CVC-ClinicDB and CVC-ColonDB test Dice of the proposed GAD-MambaUNet capacity variants are reported in Table~\ref{tab:reference_results}; results on the remaining datasets will be added after the corresponding runs are completed under the same protocol.

\subsection{Implementation Protocol}

For the reference baselines, we retain the dataset splits and evaluation pipeline of MK-UNet~\cite{rahman2025mkunet}. Images are resized to $256\times256$ for BUSI and ISIC2018, and to $352\times352$ for CVC-ClinicDB and CVC-ColonDB. The data are partitioned into training, validation, and test subsets using an 80:10:10 split.

For GAD-MambaUNet, we use AdamW with an initial learning rate of $5\times10^{-4}$, weight decay of $10^{-4}$, and cosine learning-rate decay to $10^{-6}$. Models are trained for 400 epochs with a batch size of 8, multi-scale factors $\{0.75,1.0,1.25\}$, gradient clipping at 0.5, and data augmentation enabled. The segmentation objective is the sum of weighted binary cross-entropy and weighted IoU losses. Each configuration is repeated over five runs with consecutive random seeds. DINOv3 distillation starts at epoch 10 and is linearly warmed up for 20 epochs; GAD starts at epoch 30 and controls the distillation weight until epoch 360. We use the MK-UNet evaluation script and reporting convention for model selection and final test evaluation. The teacher branch, feature projection head, and GAD coefficient update are removed at inference, so deployment cost is determined by the student network alone.
...
\begin{table*}[t]
    ...
    \begin{tabular}{lcccccccc}
        ...
        \midrule
        GAD-MambaUNet-Tiny (Ours) & 0.076 & 0.051 & -- & 94.43 & -- & 92.47 & 83.20 & -- \\
        GAD-MambaUNet-Small (Ours) & 0.199 & 0.117 & -- & 94.40 & -- & 93.85 & 90.17 & -- \\
        GAD-MambaUNet-Base (Ours) & 0.493 & 0.291 & -- & \textbf{94.90} & -- & 94.04 & 91.24 & -- \\
        GAD-MambaUNet-Medium (Ours) & 1.839 & 0.955 & -- & 94.71 & -- & 94.07 & 91.35 & -- \\
        GAD-MambaUNet-Large (Ours) & 5.871 & 3.325 & -- & 94.88 & -- & \textbf{94.70} & \textbf{92.16} & -- \\
        \midrule
        GAD-MambaUNet-Tiny + DINOv3 (Ours) & 0.076 & 0.051 & -- & -- & -- & -- & 86.54 & -- \\
        GAD-MambaUNet-Small + DINOv3 (Ours) & 0.199 & 0.117 & -- & -- & -- & -- & 91.51 & -- \\
        GAD-MambaUNet-Base + DINOv3 (Ours) & 0.493 & 0.291 & -- & -- & -- & -- & 91.96 & -- \\
        GAD-MambaUNet-Medium + DINOv3 (Ours) & 1.839 & 0.955 & -- & -- & -- & -- & \textbf{93.08} & -- \\
        GAD-MambaUNet-Large + DINOv3 (Ours) & 5.871 & 3.325 & -- & -- & -- & -- & 93.01 & -- \\
        \bottomrule
    \end{tabular}
    }
\end{table*}
