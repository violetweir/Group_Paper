...
class GroupedSS2D(SS2D):
    """
    基于并行通道切分 (PVM Layer) 思想优化的 SS2D。
    通过 Group Convolution 机制，在无 Python for 循环的情况下，并行处理切割后的 Mamba 状态空间。
    """
    def __init__(self, *args, num_chunks=4, **kwargs):
        # 截获切分数量
        self.num_chunks = num_chunks
        # 强制走 v2 的初始化路径（支持 cross2d 扫描）
        super().__init__(*args, **kwargs)

    def __initv2__(self, **kwargs):
        # 1. 继承原有初始化（完成外层 in_proj, conv2d, out_proj 等逻辑）
        ...
        k_group = kwargs.get("k_group", 4) # 默认 4 个扫描方向

        d_inner = int(ssm_ratio * d_model)
        assert d_inner % self.num_chunks == 0, f"内部通道 d_inner({d_inner}) 必须能被 num_chunks({self.num_chunks}) 整除"
        
        self.chunk_d_inner = d_inner // self.num_chunks

        # 步长秩 (dt_rank) 按比例下调，保持极低参数量
        if dt_rank == "auto":
            self.chunk_dt_rank = max(1, math.ceil((d_model / self.num_chunks) / 16))
        else:
            self.chunk_dt_rank = max(1, dt_rank // self.num_chunks)

        # 2. 魔改核心映射层 (为分组并行重写权重尺寸)
        # 总分组数 = 方向数(4) * 切分块数(num_chunks)
        total_groups = k_group * self.num_chunks
        out_per_group = self.chunk_dt_rank + d_state * 2

        # 重新定义 x_proj (用于生成 dt, B, C)
        self.x_proj_weight = nn.Parameter(torch.empty(total_groups * out_per_group, self.chunk_d_inner, 1))
        nn.init.normal_(self.x_proj_weight, std=0.02)

        # 重新定义 dt_proj (用于生成实际的步长 dt)
        self.dt_projs_weight = nn.Parameter(torch.empty(total_groups * self.chunk_d_inner, self.chunk_dt_rank, 1))
        nn.init.normal_(self.dt_projs_weight, std=0.02)

        self.dt_projs_bias = nn.Parameter(torch.empty(total_groups * self.chunk_d_inner))
        nn.init.constant_(self.dt_projs_bias, 0.0)

        # 注意：原代码的 A_logs 和 Ds 大小刚好是 [K * d_inner]，
        # 在数学上等价于 [K * num_chunks * chunk_d_inner]，因此不需要重写，可以直接沿用。

    def forward_corev2(self, x: torch.Tensor = None, force_fp32=False, ssoflex=True, no_einsum=True,
        ...
        # 关键魔法：将原本的 [B, K, d_inner, L] 转换为 [B, K*C, chunk_d_inner, L]
        # 骗过后续算子，让它们认为有 K*C 个独立的方向！
        xs_grouped = xs.view(B, K * C, cD, L)

        # --- 2. 分组并行计算 x_proj ---
        x_dbl = F.conv1d(xs_grouped.contiguous().view(B, -1, L),
                         self.x_proj_weight, bias=None, groups=K * C)
        
        x_dbl = x_dbl.view(B, K * C, cR + 2 * N, L)
        dts, Bs, Cs = torch.split(x_dbl, [cR, N, N], dim=2)

        # --- 3. 分组并行计算 dt_proj ---
        dts = F.conv1d(dts.contiguous().view(B, -1, L),
                       self.dt_projs_weight, bias=None, groups=K * C)

        # --- 4. 准备传入底层 CUDA 的张量 ---
        xs_grouped = xs_grouped.contiguous().view(B, -1, L)
        dts = dts.contiguous().view(B, -1, L)
        As = -self.A_logs.to(torch.float).exp()   # 尺寸完美对齐: [K*d_inner, N] -> [K*C*cD, N]
        Ds = self.Ds.to(torch.float)              # 尺寸完美对齐: [K*d_inner] -> [K*C*cD]
        Bs = Bs.contiguous().view(B, K * C, N, L)
        Cs = Cs.contiguous().view(B, K * C, N, L)
        delta_bias = self.dt_projs_bias.view(-1).contiguous().to(torch.float)

        if force_fp32:
            xs_grouped, dts, Bs, Cs = to_fp32(xs_grouped, dts, Bs, Cs)

        # --- 5. 核心：调用 Triton/CUDA 算子 ---
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
            backend = "torch" if u.device.type == "cpu" else ("oflex" if ssoflex else "mamba")
            return selective_scan_fn(u, delta, A, B, C, D, delta_bias, delta_softplus, ssoflex, backend=backend)

        # CUDA 算子会根据传入的 Bs (维度1为 K*C) 自动进行独立的切片并行推导，性能拉满！
        ys = selective_scan(xs_grouped, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus)

        # --- 6. 还原回正常 SS2D 形状并合并 ---
        # 结果从 [B, K*C, cD, L] 拼合回 [B, K, d_inner, H, W]
        ys = ys.view(B, K, D, H, W).contiguous()

        if K == 4:
            y = cross_merge_fn(ys, in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)
        else:
            y = cross_merge_fn_k2(ys, in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)

        # 整理输出维度
        y = y.view(B, -1, H, W).contiguous()
        if not self.channel_first:
            y = y.view(B, -1, L).transpose(1, 2).view(B, H, W, -1).contiguous()
        y = self.out_norm(y)
